    ...
    run: async(client, message, args) => {
      ...
      const row = new Discord.MessageActionRow()
        ...
        const embed = await message.reply({ embeds: [new Discord.MessageEmbed()
          .setTitle(`${config.get(`title`)} | Criação de Gift Card <a:wishzada_raio:1036400111275757628>`)
          .addField(`<:caixa:1071265643812356179> Estoque:`, `Nenhum...`)
          .addField(`<:ticketlog:1066123441809399880> Código:`, `${gerado}`)
        ...
        const interação = embed.createMessageComponentCollector({ componentType: "BUTTON", });
          interação.on("collect", async (interaction) => {
           ...
           if (interaction.customId === "concluir") {
             ...
             message.channel.send(`<a:presente_gift:1066353855882870924> | Envie os novos estoques no chat!`).then(msg => {
             ...
             collector.on("collect", message => {
               ...
               for (let i = 0; i != estoque.length; i++) {
                 ...
                 if(i + 1 === estoque.length) {
                   ...
                   msg.edit(`<:certo1:1066123147071467630> | Gift \`${gerado}\`\ Criado com sucesso!`)
                   const embednew = new Discord.MessageEmbed()
                     .setTitle(`${config.get(`title`)} | Criação de Gift Card`)
                     .addField(`<:caixa:1071265643812356179> Estoque:`, `${texto}`)
                     .addField(`<:ticketlog:1066123441809399880> Código:`, `${gerado}`)
